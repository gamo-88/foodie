    ...
    <main className='container'>
       <section className="row d-flex justify-content-center g-5 p-3">
            <div className="col-lg-6 " style={{
                ...
                <img src={`/${currentPlat.image}`} alt="" className='img-fluid w-100' />
            </div>
            <div className="col-lg-6">
                <span className='fs-4 d-block'>{currentPlat.name}</span>
                <span className='fs-4 d-block'><span className='rating text-danger'><StarRateRoundedIcon/><StarRateRoundedIcon/><StarRateRoundedIcon/><StarRateRoundedIcon/><StarOutlineRoundedIcon/></span></span>

                <span className='fs-4'><span className='text-decoration-line-through text-secondary me-3'>{currentPlat.price * 30/100 +currentPlat.price} FCFA</span><span className='text-success'> {currentPlat.price} FCFA</span></span>
                <span className='fs-6 d-block'>{currentPlat.description}</span>
                <button  className='btn btn-primary mt-4' onClick={()=>{handelAjoutfood(currentPlat._id)}} >Ajouter au panier</button>
    </div>
        </section>{
            user.isConnected? (
                <aside>
        ...
        <section className="row">
            <div className="entryComments col-lg-5">
                <form onSubmit={(e)=>{handelComments(e)}}>
                <textarea className='w-100 h-100 mb-2' name="comments" value={commentsVal}  id="comments" placeholder='Apropos du produit ou autres...' onChange={(e)=>{setCommentsVal(e.target.value)}}
                
                ></textarea>
                <button type='submit' className='btn btn-primary mb-3' disabled={!commentsVal.length}>Envoyer</button>
                </form>
            </div>
<div className="col-lg-7">
{COMMENTS.length === 0 ?(
    
    <p className='display-6'>Aucun commentaire sur le produit pour l'instant</p>
    ...
    COMMENTS.map((com, index)=>{
        return (
                ...
                <div className="commentDesc mb-2">
                    {com.comments}
                </div>
                </div>
